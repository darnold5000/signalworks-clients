  ...
  const body = `
    <article style="max-width: 820px; margin: 0 auto; font-family: Arial, sans-serif; color: #121212;">
      ...
      <section style="margin-bottom: 3rem; border: 1px solid #e2e0da; border-radius: 12px; padding: 1.5rem;">
        ...
        ${detail.searchVisibility?.status === "completed" && detail.searchVisibility.summary ? `<p style="font-size: 32px; margin: 1rem 0 0;"><strong>${Math.round(detail.searchVisibility.score ?? 0)}/100</strong></p><p style="color: #555; line-height: 1.6;">We checked ${discoveryCount} non-branded discovery ${discoveryCount === 1 ? "search" : "searches"} potential customers may use when looking for the services you offer. We also checked ${brandedCount} branded ${brandedCount === 1 ? "search" : "searches"} for the business name, shown separately below. ${detail.searchVisibility.summary.firstPageCount} produced a first-page ranking and ${detail.searchVisibility.summary.notFoundCount} were not found in the top 30.</p><h3 style="font-size: 20px;">Top search opportunities</h3><p style="color: #555; line-height: 1.6;">Search opportunities combine estimated local search demand with current Google visibility. High-demand searches where the business has little or no visibility are prioritized first.</p>${discoveryResults.slice().sort((a, b) => (b.opportunityScore ?? -1) - (a.opportunityScore ?? -1)).slice(0, 3).map((result, index) => { const ranking = result.position ? `#${result.position}` : "Not in top 30"; const interpretation = result.monthlySearchVolume == null || result.demandLevel === "unavailable" ? "" : result.position == null ? "This is a meaningful customer search, but the website was not visible in the top 30 results we checked." : result.position > 10 ? "The website is visible, but moving closer to the first page could make this opportunity more valuable." : "The website is already visible for this search, so maintaining and improving this position could protect its value."; return `<p style="border-bottom: 1px solid #eee; padding: 8px 0;"><strong>${index + 1}. ${escapeHtml(formatSearchQuery(result.query, result.resolvedLocationName ?? result.location))}</strong>${index === 0 ? ` <span style="font-size: 11px; color: #666; text-transform: uppercase;">Biggest search opportunity</span>` : ""}<br><span style="color: #555;">${escapeHtml(result.position ? ranking : ranking)}${result.monthlySearchVolume == null ? "" : ` · ${demandText(result.demandLevel, result.monthlySearchVolume)} · ${escapeHtml(result.opportunityLabel ?? "Opportunity")}`}${interpretation ? ` · ${escapeHtml(interpretation)}` : ""}</span></p>`; }).join("")}<h3 style="font-size: 20px;">Discovery visibility</h3><p style="color: #555;">What are customers searching for — and where do you appear?</p><table style="width: 100%; border-collapse: collapse; margin-top: 1rem;"><thead><tr><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Customer search</th><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Google position</th><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Search demand <span title="Estimated monthly Google searches for this term in the selected market.">ⓘ</span></th><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Evidence</th></tr></thead><tbody>${discoveryResults.map((result) => `<tr><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${escapeHtml(formatSearchQuery(result.query, result.resolvedLocationName ?? result.location))}</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${result.position ? `#${result.position}` : "Not in top 30"}</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${demandText(result.demandLevel, result.monthlySearchVolume)}</td><td style="padding: 8px 0; border-bottom: 1px solid #eee; font-size: 12px; color: #555;">${formatLocationName(result.resolvedLocationName ?? result.location) ? `${escapeHtml(formatLocationName(result.resolvedLocationName ?? result.location)!)}` : "Location not recorded"} · ${escapeHtml(result.checkedAt)} · Top ${result.resultDepth ?? 30} · ${result.position != null ? `#${result.position}` : "Not found"}${result.rankingUrl ? ` · ${escapeHtml(result.rankingUrl)}` : ""}</td></tr>`).join("")}</tbody></table>` : `<p style="color: #555; line-height: 1.6;">Search visibility could not be measured for this report. SEO Setup checks show whether your website is prepared for search, but we do not claim Google rankings without verified search data.</p>`}
      </section>

      <section style="margin-bottom: 3rem; border: 1px solid #e2e0da; border-radius: 12px; padding: 1.5rem;">
        <p style="text-transform: uppercase; letter-spacing: 0.16em; font-size: 11px; color: #666;">Local search</p>
        <h2 style="font-size: 24px; font-weight: 500;">Can nearby customers find your business?</h2>
        ${detail.localSearch?.status === "not_applicable" ? `<p style="color: #555;">Google Maps &amp; Local Search: Not applicable.</p>` : detail.localSearch?.status === "completed" && detail.localSearch.summary ? `<p style="font-size: 32px; margin: 1rem 0 0;"><strong>${Math.round(detail.localSearch.score ?? 0)}/100</strong></p><p style="color: #555; line-height: 1.6;">We found the business in ${detail.localSearch.summary.foundCount} of ${detail.localSearch.summary.queriesAnalyzed} Google local searches. ${detail.localSearch.summary.topThreeCount} ranked in the top three.</p><table style="width: 100%; border-collapse: collapse; margin-top: 1rem;"><thead><tr><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Customer search</th><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Google local position</th><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">How we checked</th></tr></thead><tbody>${detail.localSearch.results.map((result) => `<tr><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${escapeHtml(formatSearchQuery(result.query, result.resolvedLocationName ?? result.location))}</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${result.position ? `#${result.position}` : "Not found"}</td><td style="padding: 8px 0; border-bottom: 1px solid #eee; font-size: 12px; color: #555;">${escapeHtml(formatLocationName(result.resolvedLocationName ?? result.location) ?? "Location not recorded")} · ${escapeHtml(result.checkedAt)} · Top ${result.resultDepth ?? 20} · ${result.position != null ? `#${result.position}` : "Not found"}${result.resultUrl ? ` · ${escapeHtml(result.resultUrl)}` : ""}</td></tr>`).join("")}</tbody></table>` : `<p style="color: #555;">Google Maps &amp; Local Search could not be measured for this report.</p>`}
      </section>
