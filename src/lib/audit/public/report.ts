  ...
  const body = `
    <article style="max-width: 820px; margin: 0 auto; font-family: Arial, sans-serif; color: #121212;">
      ...
      <section style="margin-bottom: 3rem; border: 1px solid #e2e0da; border-radius: 12px; padding: 1.5rem;">
        ...
        ${detail.searchVisibility?.status === "completed" && detail.searchVisibility.summary ? `<p style="font-size: 32px; margin: 1rem 0 0;"><strong>${Math.round(detail.searchVisibility.score ?? 0)}/100</strong></p><p style="color: #555; line-height: 1.6;">We checked ${detail.searchVisibility.summary.queriesAnalyzed} Google searches. ${detail.searchVisibility.summary.firstPageCount} produced a first-page ranking and ${detail.searchVisibility.summary.notFoundCount} were not found in the top 30.</p><table style="width: 100%; border-collapse: collapse; margin-top: 1rem;"><thead><tr><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Customer search</th><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Google position</th><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Search demand</th><th style="text-align: left; border-bottom: 1px solid #ddd; padding: 8px 0;">Opportunity</th></tr></thead><tbody>${detail.searchVisibility.results.map((result) => `<tr><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${escapeHtml(result.query)}</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${result.position ? `#${result.position}` : "Not in top 30"}</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${result.demandLevel === "high" ? "High" : result.demandLevel === "moderate" ? "Moderate" : result.demandLevel === "low" ? "Low" : result.demandLevel === "very_low" ? "Very low" : "Not available"}</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${result.opportunityLabel === "high_priority" ? "High priority" : result.opportunityLabel === "strong_opportunity" ? "Strong opportunity" : result.opportunityLabel === "already_strong" ? "Already strong" : result.opportunityLabel === "moderate_opportunity" ? "Moderate opportunity" : "Not available"}</td></tr>`).join("")}</tbody></table>` : `<p style="color: #555; line-height: 1.6;">Search visibility could not be measured for this report. SEO Setup checks show whether your website is prepared for search, but we do not claim Google rankings without verified search data.</p>`}
      </section>
