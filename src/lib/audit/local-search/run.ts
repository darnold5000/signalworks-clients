...
export async function runLocalSearch(input: {
  ...
  if (!profile.applicable) return { status: "not_applicable", score: null, profileKey: profile.key, enteredMarket: input.enteredMarket, normalizedMarket: input.locationName, locationName: input.locationName, locationCode: input.locationCode, results: [], summary: null, checkedAt: null };
  ...
  const results = await Promise.all(queries.map(async (query) => {
    const items = await fetchGoogleLocalResults({ keyword: query, locationCode: input.locationCode, locationName: input.locationName });
    const match = items.find((item) => localBusinessMatches({ item, businessName: input.businessName, targetDomain }));
    ...
    return { query, position, found: position != null, businessName: match?.title ?? null, websiteDomain: match?.domain ?? null, resultUrl: match?.url ?? null, location: input.locationName, checkedAt };
  }));
  ...
  return { status: "completed", score: summary.score, profileKey: profile.key, enteredMarket: input.enteredMarket, normalizedMarket: input.locationName, locationName: input.locationName, locationCode: input.locationCode, results, summary, checkedAt };
}
