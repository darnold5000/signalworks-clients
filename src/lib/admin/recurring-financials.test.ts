...
describe("recurring financial reporting", () => {
  it("calculates 49.99 - 10.99 as 39.00 ongoing MRR", () => {
    const result = calculateRecurringFinancials([{ items: [recurring("plan", 4999), discount(1099)], purchasedAt: "2026-01-01T00:00:00Z" }], 0, new Date("2026-08-01T00:00:00Z"));
  ...
  it("calculates 159.99 - 50.00 as 109.99 while a temporary discount is active", () => {
    const result = calculateRecurringFinancials([{ items: [recurring("plan", 15999), discount(5000, "repeating", 6)], purchasedAt: "2026-07-29T00:00:00Z" }], 0, new Date("2026-08-27T00:00:00Z"));
    expect(result).toMatchObject({ effectiveMrrCents: 10999, discountKind: "temporary", discountPeriodsRemaining: 5 });
  });

  it("stops reducing MRR after a temporary discount expires", () => {
    const result = calculateRecurringFinancials([{ items: [recurring("plan", 15999), discount(5000, "repeating", 6)], purchasedAt: "2026-01-01T00:00:00Z" }], 0, new Date("2026-08-01T00:00:00Z"));
  ...
  it("normalizes annual and quarterly revenue and sums multiple services", () => {
    const result = calculateRecurringFinancials([{ items: [recurring("annual", 12000, "year"), { ...recurring("quarterly", 30000, "month", 3), sort_order: 1 }], purchasedAt: "2026-01-01T00:00:00Z" }]);
  ...
  it("excludes one-time charges and reduces margin by discounts before costs", () => {
    ...
    const result = calculateRecurringFinancials([{ items: [recurring("plan", 4999), discount(1099), oneTime], purchasedAt: "2026-01-01T00:00:00Z" }], 1000);
